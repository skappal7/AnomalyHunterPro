...
- Upload a CSV (large files are processed via **chunking** for stability) **or use sample data**.
...
- View **visual anomaly distribution**, a **narrative data story** explaining *why* items were flagged, and download **results & insights** as CSV/PDF.
...
with st.sidebar:
    ...
    st.markdown(
        """
    1. **Choose Data**: Upload CSV *or* use sample data.
    2. **Pick Columns**: Categorical + numeric metrics.
    3. **Select Method**: IsolationForest (global isolation) or LOF (local density).
    4. **Run Analysis**: See charts, table, and narrative.
    5. **Download**: Export results & insights as CSV/PDF.

    ℹ️ Chunking is used so large files load safely.
    """
    )
...
def load_csv_in_chunks(file, chunksize=200_000, usecols=None, downcast=True):
    """Load CSV in chunks with an indeterminate progress bar. Optionally select columns and downcast numerics."""
    try:
        chunks = []
        progress_bar = st.progress(0, text="Loading data in chunks…")
        total_rows = 0
        for i, chunk in enumerate(
            pd.read_csv(file, low_memory=False, chunksize=chunksize, usecols=usecols)
        ):
            if downcast:
            ...
            # Indeterminate-ish progress: grow slowly but steadily
...
def load_sample_data():
    ...
    for week in weeks:
        for dept in departments:
            # Department-level baseline variability
            base = np.random.randint(150, 260)
            for agent in agents:
                sales = np.random.poisson(lam=base)
                # Inject both high- and low-side anomalies (~6%)
                r = np.random.rand()
                if r < 0.03:
...
# Stage 1: If uploading, optionally pre-read a small sample for column selection
uploaded_file = None
sample_for_columns = None
usecols_selected = None

if option == "Upload CSV":
    uploaded_file = st.file_uploader(
        "📂 Upload CSV file",
        type=["csv"],
        help="Upload a CSV. Large files are processed via chunking for stability.",
    )
...
# Column selection UI (works for both sources)
...
if all_columns:
    categorical_col = st.selectbox(
        ...
        options=all_columns,
        help="Choose the column representing categories, e.g., Agents or Departments.",
    )
    # Numeric columns will be resolved after full load; use sample to filter if available
    numeric_guess = []
    if option == "Use Sample Data":
    ...
    numeric_cols = st.multiselect(
        ...
        help="Choose numeric metrics for anomaly detection.",
    )

    # Efficient load button for uploaded large files
    ...
    # If not using the efficient button (or using sample), fallback to full load for safety
    if option == "Upload CSV" and uploaded_file is not None and 'df' not in locals():
        df = load_csv_in_chunks(uploaded_file)

# If we have a dataframe, proceed
if 'df' in locals() and not df.empty and all(col in df.columns for col in ([categorical_col] if 'categorical_col' in locals() else [])):
    ...
    method = st.radio(
        ...
        horizontal=True,
        help="IsolationForest isolates outliers globally; LOF finds points in sparse local neighborhoods.",
    )
    ...
    if numeric_cols:
        ...
        if method == "IsolationForest":
            ...
            # Higher score => more normal; convert to positive anomaly score
            scores = -model.score_samples(X)
        else:
            # LOF only supports fit_predict; capture negative_outlier_factor_
            lof = LocalOutlierFactor(n_neighbors=min(35, max(5, int(np.sqrt(len(X))))), contamination='auto', n_jobs=-1)
            labels = lof.fit_predict(X)
            # Make higher score => more anomalous (positive)
        ...
        # -----------------------------
        # Visual anomaly distribution with percentage
        # -----------------------------
        ...
        fig_pie = px.pie(cdf, names="label", values="count", hole=0.5, hover_data=["percent"], title=f"Anomalies: {anomalies:,} of {total:,} (~{pct:.2f}%)")
        fig_pie.update_traces(textposition="inside", textinfo="percent+label")
        st.plotly_chart(fig_pie, use_container_width=True)

        st.caption("This chart shows the share of records flagged as anomalous. Percentages are of total analyzed rows.")

        # -----------------------------
        # Narrative data story
        # -----------------------------
        st.subheader("🧠 Narrative Insight")
        # Top categories driving anomalies
        grp = df_analysis.groupby([categorical_col, "anomaly"]).size().unstack(fill_value=0)
        grp["anomaly_rate"] = grp.get("Anomaly", 0) / (grp.sum(axis=1) + 1e-9)
        top_agents = grp.sort_values("anomaly_rate", ascending=False).head(5)

        reason_blurb = (
            "IsolationForest isolates records that require fewer random splits to isolate; "
            "such records tend to have uncommon metric combinations."
            if method == "IsolationForest"
            else "LOF compares local density—points in sparse neighborhoods relative to their neighbors are flagged."
        )

        story_lines = [
            f"Out of **{total:,}** records, **{anomalies:,} (~{pct:.2f}%)** were flagged as anomalies using **{method}**.",
            f"Why flagged? {reason_blurb}",
        ]
        # Add top drivers
        if not top_agents.empty:
            leader = top_agents.index[0]
            story_lines.append(
                f"The highest anomaly rate was observed for **{leader}** at **{(top_agents.iloc[0]['anomaly_rate']*100):.1f}%** of its records."
            )
        # Strongest individual outliers by score
        strongest = df_analysis.sort_values("anomaly_score", ascending=False).head(3)
        if not strongest.empty:
            top_examples = ", ".join(
                [f"{row[categorical_col]} (score={row['anomaly_score']:.2f})" for _, row in strongest.iterrows()]
            )
            story_lines.append(
                f"Most extreme outliers by score include: {top_examples}. Higher scores imply stronger deviations from typical patterns."
            )

        st.markdown("\n".join([f"- {s}" for s in story_lines]))

        # -----------------------------
        # Results Table with pill styling
        # -----------------------------
        st.subheader("📋 Results Table")
        view_cols = [categorical_col] + numeric_cols + ["anomaly", "anomaly_score"]
        table_df = df_analysis[view_cols].copy()

        def style_pills(val):
            if isinstance(val, str) and val.lower().startswith("anomaly"):
                return (
                    "background: linear-gradient(90deg, #ff4d4f, #ff7875); color: white; border-radius: 999px; "
                    "padding: 2px 10px; text-align:center;"
                )
            elif isinstance(val, str) and val.lower().startswith("normal"):
                return (
                    "background: linear-gradient(90deg, #1890ff, #69c0ff); color: white; border-radius: 999px; "
                    "padding: 2px 10px; text-align:center;"
                )
            return ""
        ...
        styler = (
            ...
            .applymap(style_pills, subset=["anomaly"])
            .set_properties(subset=[categorical_col], **{"white-space": "nowrap"})
        )
        st.dataframe(styler, use_container_width=True, height=420)

        # -----------------------------
        # 2D visualization (if >=2 metrics) or histogram otherwise
        # -----------------------------
